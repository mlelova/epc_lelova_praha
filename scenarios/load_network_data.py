...
def _apply_gas_co2_prices(
    ...
    """Recompute marginal costs in the technologies DataFrame for new gas/CO₂ prices.
    ...
    gas_mask = tech["fuel_type"].astype(str).str.lower().isin(["gas", "gas-ccs"])
...
def load_network_data(
    ...
    if gas_price is not None or co2_price is not None:
        print(f", gas_price={gas_price}, co2_price={co2_price}", end="")
    print()
    ...
    if gas_price is not None or co2_price is not None:
        data["technologies"] = _apply_gas_co2_prices(
            data["technologies"], gas_price, co2_price
        )
